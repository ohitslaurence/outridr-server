/**
 * Remote/local config writes — `repos.roots` (the app's onboarding flow,
 * `PUT /repos/roots`) and `token` (`outridr pair`'s token generation). Split
 * from `lib/config.mjs` (which only reads at startup) because writing is a
 * different concern: it validates, persists atomically, and mutates the live
 * in-process config so the change is visible to the next request/step in
 * this process.
 */
...
/**
 * Ensures `config.token` is set, generating and persisting a strong one if
 * not. Idempotent: if `config.token` is already set, returns it unchanged
 * without touching the config file — pairing never clobbers an existing
 * token. Otherwise generates `randomBytes(32).toString("hex")` (a 64-hex-char
 * token), writes it into the config file atomically (write-then-rename,
 * mode 0o600, preserving every other key — mirrors `saveRepoRoots`), mutates
 * `config.token` in place, and returns the new token.
 */
export async function saveToken(config) {
  if (config.token) {
    return config.token;
  }
  ...
  if (existsSync(CONFIG_PATH)) {
    try {
      ...
      throw new Error(`config file at ${CONFIG_PATH} is not valid JSON: ${error.message}`);
    }
  }
  file.token = token;
