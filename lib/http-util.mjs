...
export function hostAllowed(host) {
  ...
  let name;
  if (host.startsWith("[")) {
    // Bracketed IPv6 literal, optionally with :port — "[::1]" or "[::1]:8080".
    // Reject a missing "]" or any trailing garbage after it, so the guard
    // never accepts more than a bare "[literal]" / "[literal]:port".
    const end = host.indexOf("]");
    const rest = end === -1 ? null : host.slice(end + 1);
    if (end === -1 || (rest !== "" && !/^:\d+$/.test(rest))) {
      return false;
    }
    name = host.slice(1, end);
  } else if (host.indexOf(":") === host.lastIndexOf(":")) {
    // At most one colon: a bare name or "name:port". A bracketless IPv6
    // literal always has >= 2 colons, so a trailing-:port strip here can never
    // eat an address group.
    name = host.replace(/:\d+$/, "");
  } else {
    // Two or more colons, unbracketed: a bracketless IPv6 literal, which
    // cannot carry a port (RFC 3986/7230 require brackets for that). Verbatim.
    name = host;
  }
