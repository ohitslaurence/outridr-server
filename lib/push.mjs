...
const EXPO_PUSH_URL = new URL(
  process.env.OUTRIDR_EXPO_PUSH_URL ?? "https://exp.host/--/api/v2/push/send",
);

/** Expo caps one send request at 100 messages. */
export const EXPO_BATCH_LIMIT = 100;

// Any tokenless tailnet peer can call /push/register; without a cap the
// persisted store grows without bound. 50 is generous for a personal tool.
...
export class PushTokenStore {
  ...
  add(token, device) {
    if (!this.tokens.has(token) && this.tokens.size >= MAX_PUSH_TOKENS) {
      const oldest = [...this.tokens.values()].sort((a, b) =>
        a.registeredAt < b.registeredAt ? -1 : a.registeredAt > b.registeredAt ? 1 : 0,
      )[0];
      if (oldest) {
        this.tokens.delete(oldest.token);
        console.error("outridr: push token cap reached; evicted oldest");
      }
    }
      ...
      if (result) {
        for (const agent of agents) {
          ...
          if (config.push.notifyOn.includes(agent.agent_status)) {
            const title = agent.agent_status === "blocked" ? "Agent needs you" : "Agent finished";
...
function postToExpo(url, payload, onData) {
  ...
  request.on("error", (error) =>
    console.error(`outridr: expo request to ${url.pathname} failed: ${error.message}`),
  );
  request.end(payload);
}
