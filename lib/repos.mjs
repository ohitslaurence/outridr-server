 ...
 * is any directory containing a `.git` DIRECTORY. A `.git` FILE marks a
 * linked worktree or submodule checkout, which are deliberately excluded —
 * task worktrees are workspace artifacts, not repos to start tasks from.
 * Scanning stops at a repo: nested repos inside a checkout are the repo's
 * own business.
 */
...
async function listSubdirs(dir) {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && !entry.name.startsWith(".") && !SKIP_NAMES.has(entry.name),
    )
