...
function handleHttpUnsafe(config, pushTokens, req, res) {
  // The Host allowlist is a DNS-rebinding defense that matters only when there
  // is no token: a configured token already blocks any cross-origin/rebinding
  // browser request (it can't present the secret). Enforcing it with a token
  // set would wrongly 421 the native app, which legitimately addresses this
  // machine by its short MagicDNS hostname (not an IP or *.ts.net FQDN).
  if (!config.token && !hostAllowed(req.headers.host)) {
