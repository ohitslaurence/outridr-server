...
export function uninstallService() {
  if (platform() === "linux") {
    ...
    run("launchctl", ["unload", LAUNCHD_PLIST_PATH], true);
...
# Child processes (configured exec/repos commands, #!/usr/bin/env node
# shebangs) need node and user bins on PATH — systemd's default is bare.
...
`;
  ...
  run("launchctl", ["unload", LAUNCHD_PLIST_PATH], true);
  run("launchctl", ["load", LAUNCHD_PLIST_PATH]);
