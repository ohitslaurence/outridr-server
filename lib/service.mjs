...
export function installService() {
  if (platform() === "linux") {
    ...
    console.error(
      `outridr: unsupported platform ${platform()} — run \`outridr serve\` under your own supervisor`,
    );
    process.exit(1);
  }
}
...
/** Writes a tiny POSIX-sh launcher onto PATH that execs the install-time Node + entrypoint. */
...
/** Removes the CLI launcher, but only if it's the one we wrote (guarded by LAUNCHER_MARKER). */
...
function servicePath() {
  ...
  if (process.platform === "darwin") {
    // The Tailscale.app CLI isn't on PATH by default (App Store / sandboxed
    // install); add its bundle dir so the installed service can find it too.
