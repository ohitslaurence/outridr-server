...
function bridgeWebSocket(config, ws, head) {
  ...
  // Honor backpressure. A single 4 MB message can carry millions of one-line
  // requests; each over the in-flight cap synchronously emits a busy-error
  // frame. Ignoring ws.write's return value would let those responses buffer
  // without bound for a slow-reading peer. Instead, when the socket's write
  // buffer fills we pause both inbound reads and the dispatch pump until the
  // outbound queue drains, so buffered output stays bounded by the socket
  // high-water mark rather than by the attacker's message size.
  const resumeWrites = () => {
    if (!wsOpen) {
      return;
    }
  ...
  const sendLineToWs = (line) => {
    ...
    if (!ws.write(encodeFrame(0x1, Buffer.from(line, "utf8"))) && !wsPaused) {
      wsPaused = true;
      ws.pause();
      ws.once("drain", resumeWrites);
    }
  };

  const pumpPending = () => {
    while (wsOpen && !wsPaused && pending.length > 0) {
      dispatchRequestLine(pending.shift());
    }
  };
